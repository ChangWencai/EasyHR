<template>
  <div class="page-view">
    ...
    <div class="stats-grid" v-loading="loading">
      <ComplianceStatCard :value="stats.anomaly_employee_count" label="异常人数" icon="WarningFilled" icon-class="icon--anomaly" />
      <ComplianceStatCard :value="stats.total_late_count" label="迟到总次数" icon="Clock" icon-class="icon--late" />
      <ComplianceStatCard :value="stats.total_absent_days" label="缺勤总天数" icon="CloseBold" icon-class="icon--absent" />
    </div>
    ...
    >
      ...
      <el-table-column label="迟到(次)" min-width="90" align="right">
        <template #default="{ row }"><span class="num-val" :class="row.late_count > 0 ? 'num-val--warning' : ''">{{ row.late_count }}</span></template>
      </el-table-column>
      <el-table-column label="早退(次)" min-width="90" align="right">
        <template #default="{ row }"><span class="num-val">{{ row.early_leave_count }}</span></template>
      </el-table-column>
      ...
      <el-table-column label="异常次数" min-width="90" align="right">
        <template #default="{ row }"><span class="num-val">{{ row.anomaly_count }}</span></template>
      </el-table-column>
...
const page = ref(1)
const pageSize = 20
const stats = ref({ anomaly_employee_count: 0, total_late_count: 0, total_absent_days: 0 })
...
async function load(p = 1) {
  ...
  try {
    ...
    const { data } = await attendanceApi.getComplianceAnomaly({ year_month: selectedMonth.value, dept_ids: deptIds, page: p, page_size: pageSize })
...
.icon--late { background: linear-gradient(135deg, #FEF3C7, #FDE68A); color: #F59E0B; }
...
.num-val { font-weight: 600; font-family: 'SF Mono', Monaco, monospace; &--warning { color: var(--warning); } &--danger { color: var(--danger); } }
</style>
