...
// FIX (checker): status parameter added for D-09-11 filter support
func (r *Repository) ListTodos(ctx context.Context, orgID int64, status string, page, pageSize int) ([]TodoItem, int64, error) {
	...
	// D-09-11: status filter — empty means all statuses (including terminated)
	if status != "" {
...
func (r *Repository) SearchTodos(ctx context.Context, orgID int64, keyword string, status string, page, pageSize int) ([]TodoItem, int64, error) {
	...
	// D-09-11: status filter in search
	if status != "" {
		query = query.Where("status = ?", status)
	}
...
func (r *Repository) FilterByDateRange(ctx context.Context, orgID int64, startDate, endDate time.Time, status string, page, pageSize int) ([]TodoItem, int64, error) {
	...
	// D-09-11: status filter in date range
	if status != "" {
		query = query.Where("status = ?", status)
	}
...
// FIX (checker): status field added for D-09-11 filter
...
// ListTodos 查询待办列表
// FIX (checker): status passed through to repository for D-09-11 filter
...
注意：main.go 中已经存在 dashboard router 注册，需要确认 todo router 的注册方式。检查 main.go 中的路由注册模式，在同一位置添加 todo router。
  ...
  <done>
    ...
    GET /api/v1/todos 支持 keyword/start_date/end_date/status/page/page_size 查询参数
  ...
  <acceptance_criteria>
    ...
    - [ ] ListTodosRequest has Status field (D-09-11)
    - [ ] ListTodos repository method accepts status param and applies WHERE clause when non-empty (D-09-11)
  </acceptance_criteria>
</task>

<task type="auto">
  <name>Task 2: 前端待办列表页 TodoListView.vue + api/todo.ts（含状态筛选）</name>
...
// FIX (checker): status param added for D-09-11 filter
...
FIX (checker): status el-select filter added for D-09-11 — terminated todos must be viewable via filter.
  ...
  <div class="todo-list-view">
    ...
    <div class="filter-card section">
      <el-form :inline="true" :model="filters">
        <!-- FIX (checker): D-09-11 status filter — allows viewing terminated todos -->
...
// FIX (checker): D-09-11 — status filter added; allows admin to view terminated todos
...
- 已终止: #8C8C8C
  ...
  <verify>
    <automated>grep -n "listTodos\|pinTodo\|exportTodos\|TodoListView\|filters.status" frontend/src/api/todo.ts frontend/src/views/todo/TodoListView.vue frontend/src/router/index.ts | head -20</automated>
  </verify>
  <done>
    /todo 页面展示完整待办列表，支持关键字搜索、日期范围筛选和状态筛选
    待办列表按 is_pinned DESC, sort_order DESC, created_at DESC 排序
    置顶按钮可切换置顶/取消置顶
    导出按钮下载 Excel 文件（xlsx格式）
    分页支持20/50/100条每页
    状态筛选可查看已终止的待办事项（D-09-11）
  </done>
  <acceptance_criteria>
    ...
    - [ ] Status el-select filter present with options: 全部/待办/已完成/已终止 (D-09-11)
    - [ ] listTodos params include status field (D-09-11)
  </acceptance_criteria>
</task>
...
6. `grep -n "filters.status\|el-select.*status" frontend/src/views/todo/TodoListView.vue` confirms status filter present (D-09-11)
</verification>

<success_criteria>
- GET /api/v1/todos 返回分页待办列表，支持 keyword/search 和 date range 和 status 筛选
- PUT /api/v1/todos/:id/pin 切换置顶状态
- GET /api/v1/todos/export 返回 Excel xlsx 文件
- GET /api/v1/carousels 返回轮播图列表（实际调用 repo.ListCarousels）
- /todo 页面可访问，el-table 展示全部列表字段
- 搜索/筛选（包含状态筛选）/置顶/导出功能全部可用
- 日期筛选限制最多60天
- 所有 API 正确使用 org_id 租户隔离
- 状态筛选支持全部/待办/已完成/已终止，可查看已终止待办（D-09-11）
</success_criteria>
