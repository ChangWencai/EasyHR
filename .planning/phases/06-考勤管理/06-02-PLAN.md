...
func (r *AttendanceRepository) ListEmployeesByIDs(orgID int64, ids []int64) ([]Employee, error) {
    var emps []Employee
    if len(ids) == 0 {
        return emps, nil
    }
    err := r.db.Scopes(r.orgScope(orgID)).Where("id IN ?", ids).Find(&emps).Error
    return emps, err
}
...
func (s *AttendanceService) GetClockLive(ctx context.Context, orgID int64, date string, departmentID *int64, page, pageSize int) (*ClockLiveResponse, error) {
    ...
    // 5. JOIN employee 表获取姓名和部门（修复: ClockLive page 显示空白姓名 per checker）
    employeeIDs := make([]int64, 0, len(clockMap))
    for empID := range clockMap {
        employeeIDs = append(employeeIDs, empID)
    }
    empMap := make(map[int64]struct{ Name, DepartmentName string })
    if len(employeeIDs) > 0 {
        emps, _ := s.repo.ListEmployeesByIDs(orgID, employeeIDs)
        for _, e := range emps {
            empMap[e.ID] = struct{ Name, DepartmentName string }{Name: e.Name, DepartmentName: e.DepartmentName}
        }
    }

    // 6. 构建 ClockLiveItem 列表
...
// AttendanceManualStats 手动修正记录（ATT-08: 管理员可手动修改假勤统计数据 per must_haves）
...
// UpdateLeaveStats 持久化手动修正数据（ATT-08: 已实现 per acceptance_criteria）
func (s *AttendanceService) UpdateLeaveStats(ctx context.Context, orgID int64, employeeID int64, yearMonth string, req *UpdateLeaveStatsRequest) error {
    stats := &AttendanceManualStats{
        BaseModel: model.BaseModel{OrgID: orgID, UpdatedBy: orgID},
        EmployeeID: employeeID,
        YearMonth: yearMonth,
    ...
    return s.db.Clauses(clause.OnConflict{
        Columns: []clause.Column{{Name: "employee_id"}, {Name: "year_month"}},
        DoUpdates: clause.AssignmentColumns([]string{"leave_days", "business_days", "outside_days", "makeup_count", "shift_swap_count", "overtime_hours", "updated_by", "updated_at"}),
    }).Create(stats).Error
}
```
...
async function handleInviteClock(row: ClockLiveRecord) {
  try {
    const today = new Date().toISOString().split('T')[0]
    const clockTime = `${today}T${new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}:00Z`
