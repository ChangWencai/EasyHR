...
must_haves:
  ...
  key_links:
    ...
    - from: "service.go"
      ...
      via: "ruleEngine.IsHoliday(date) per D-12-05"
---
...
<task type="auto">
  <name>Task 1: Add ComplianceReport DTO types to dto.go</name>
...
<task type="auto">
  <name>Task 2: Add compliance query methods to repository.go</name>
...
<task type="auto">
  <name>Task 3: Add compliance report service methods to service.go</name>
...
func (s *AttendanceService) GetComplianceOvertime(ctx context.Context, orgID int64, req *ComplianceReportRequest, page, pageSize int) (*ComplianceOvertimeResponse, error) {
	...
	for _, a := range approvals {
		if a.ApprovalType == ApprovalTypeOvertime && a.Status == ApprovalStatusApproved {
			...
			category := ruleEngine.ClassifyOvertimeCategory(a.StartTime, a.EndTime)
			switch category {
			case "holiday":
...
Also ensure `import "strconv"` is in service.go.
  ...
  <done>All 5 compliance service methods exist in service.go with correct logic (0.5h rounding, holiday/weekday/weekend classification, anomaly thresholds, Excel export)</done>
</task>

<task type="auto">
  <name>Task 4: Register compliance routes and add handler methods in handler.go</name>
...
|----------|-------------|
| client → API | Untrusted input (year_month, dept_ids query params) crosses here |
