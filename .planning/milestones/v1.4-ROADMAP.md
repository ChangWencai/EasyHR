...
**Plan list**:
- [x] 11-01-PLAN.md — 后端合同管理 API（模板 CRUD + PDF 生成 + 签署流程）— **SHIPPED**
- [x] 11-02-PLAN.md — 前端合同管理页面（模板列表/编辑/预览 + 员工签署）— **SHIPPED**
...
**Plan list**:
- [ ] 12-01-PLAN.md — 后端合规报表 API（加班/请假/异常/月度汇总 + Excel 导出）— 待执行
- [ ] 12-02-PLAN.md — 前端合规报表页面（4 个 Vue 页面 + 侧边栏菜单 + 共享组件）— 待执行
...
1. 岗位管理：支持创建/编辑/删除岗位（名称、所属部门、职级排序），岗位可跨部门复用
2. 组织架构图增强：可视化展示部门→岗位→员工三级树形结构，支持搜索高亮、节点展开/折叠、拖拽调整部门层级
3. 部门管理完善：架构图内支持内联编辑部门名称、排序、删除（含员工转移提示）
4. 员工关联：员工创建/编辑时通过下拉选择部门和岗位，替代当前自由文本输入

**Plans**: 14-01 (backend: Position CRUD + Department enhance), 14-02 (frontend: OrgChart enhance + position dropdown)
**UI hint**: yes
...
| 12. 考勤合规报表 | v1.4 | 0/2 | Not started | - |
| 13. 工资合规 | v1.4 | 0/TBD | Not started | - |
| 14. 组织架构图（部门+岗位管理） | v1.4 | 2/2 | Complete | 2026-04-21 |
