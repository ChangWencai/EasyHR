...
func GeneratePayrollVoucher(ctx context.Context, voucherSvc *VoucherService, accountSvc *AccountService, orgID, userID, payrollRecordID int64, wageTotal decimal.Decimal) (*Voucher, error) {
	// Find the 管理费用-工资 account (code 660201 for sub-account, or 6602 for parent)
	debitAcct, err := accountSvc.findAccountByCode(orgID, "660201")
	if err != nil {
		// Fallback to parent account 6602
		debitAcct, err = accountSvc.findAccountByCode(orgID, "6602")
		if err != nil {
	...
	creditAcct, err := accountSvc.findAccountByCode(orgID, "280101")
	if err != nil {
		// Fallback to parent account 2801
		creditAcct, err = accountSvc.findAccountByCode(orgID, "2801")
		if err != nil {
	...
	period, err := accountSvc.GetOrCreateCurrentPeriod(orgID)
	if err != nil {
	...
	voucher, err := voucherSvc.CreateVoucher(orgID, userID, req)
	if err != nil {
