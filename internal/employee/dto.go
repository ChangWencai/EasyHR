...
type CreateEmployeeRequest struct {
	...
	// 社保参保信息（可选，创建员工时一并登记参保）
	SICityCode          *int64   `json:"si_city_code"`           // 参保城市编码
	SIBaseAmount        *float64 `json:"si_base_amount"`         // 社保缴费基数（为空则用 Salary）
	SIStartMonth        string   `json:"si_start_month"`         // 参保起始月 YYYY-MM
	HousingFundBase     *float64 `json:"housing_fund_base"`      // 公积金缴费基数（为空则用 SIBaseAmount）
	// 绩效系数（可选，创建员工时一并初始化）
	PerformanceCoefficient *float64 `json:"performance_coefficient"` // 绩效系数，默认 1.0000
}
