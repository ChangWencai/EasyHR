...
func StartScheduler(rdb *redis.Client, svc *Service) (gocron.Scheduler, error) {
	...
	// Redis 可用时启用分布式锁
	if rdb != nil {
		locker := newRedisLocker(rdb, "easyhr:tax:")
	...
	// 每日 08:00 CST 扫描个税申报到期提醒
	_, err = s.NewJob(
		gocron.DailyJob(1, gocron.NewAtTimes(gocron.NewAtTime(8, 0, 0))),
		gocron.NewTask(func() {
			svc.CheckDeclarationReminders()
		}),
		gocron.WithName("tax-declaration-due-check"),
	)
	if err != nil {
		return nil, fmt.Errorf("create declaration due job: %w", err)
	}
